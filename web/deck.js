...
function drawDeckShadow(uiSide, manaOrCard, cards) {
    anim.fire('drawDeckShadow', { uiSide, manaOrCard, cards });
}
...
function cardDrawStage1(pl, div) {
    drawAnim[pl].enqueue('deckDraw', { pl, deckDiv: div });
}
...
async function cardDrawStage2(card, pl, justPlayed = false) {
    drawAnim[pl].enqueue('addToHand', { card, pl, justPlayed });
}
...
function oneWithNothing(pl) {
    ...
    anim.enqueue('discardHand', { pl, cards: ch, rects });
}
...
function die(card) {
    ...
    anim.enqueue('cardDeath', { card, pl, pos: card.pos });
}

function move(card, pos, pl, target) {
    if (target == pos) return 0;
    anim.enqueue('move', { card, fromPos: pos, pl, toPos: target });

    // Return expected duration for callers that still need it
    const durations = [-1, 150, 300, 400];
    return durations[Math.min(durations.length - 1, Math.abs(target - pos))];
}
...
for (let i = 0; i < sigilFilters.length; i++) {
    ...
    sigilFilters[i].addEventListener("mouseenter", function () {
