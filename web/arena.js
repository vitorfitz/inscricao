...
let sigilEstimates = {};
{
    ...
    se[s_extra_attack.id] = {
        uselessAt0: true, ea: (pe) => {
            if (has(pe, s_flying)) pe.offense *= 2.03;
    ...
    se[s_death_touch.id] = {
        uselessAt0: true, mod: (pe) => {
            ...
            else pe.dt = 1 + 27 / ((pe.attack + 2) ** 3 / 2);
            pe.offense *= pe.dt;
        }, boost: (pe) => {
    ...
    se[s_sidestep.id] = {
        boost: (pe) => {
            if (!has(pe, s_burrow) && !has(pe, s_guardian) && !has(pe, s_sq_spawner) && !has(pe, s_skele_spawner) && !has(pe, s_push) && !has(pe, s_dam) && !(has(pe, s_brittle) && pe.attack > 0)) pe.additive += pe.defense / 8;
        }
    };
    se[s_push.id] = {
        boost: (pe) => {
            if (!has(pe, s_burrow) && !has(pe, s_guardian) && !has(pe, s_sq_spawner) && !has(pe, s_skele_spawner) && !(has(pe, s_brittle) && pe.attack > 0)) pe.additive += pe.defense / 8;
        }
    };
    se[s_guardian.id] = {};
    se[s_free_sac.id] = {
    ...
    se[s_sniper.id] = {
        uselessAt0: true, boost: (pe) => {
            if (pe.offense != 0) {
                ...
                else pe.additive += (pe.offense) * 0.15;
            }
        }
    };
    se[s_dam.id] = {
...
function powerEstimate(atk, hp, sigils = []) {
    let state = {
        attack: atk,
        hp: hp,
        offense: atk,
    ...
    let s = [];
    for (let i = 0; i < state.sigils.length; i++) {
        const se = sigilEstimates[state.sigils[i].id];
        if (se) {
            s.push(se);
        }
    }
    ...
    for (let i = 0; i < s.length; i++) {
        if (s[i].mod) s[i].mod(state);
    }
    for (let i = 0; i < s.length; i++) {
        if (s[i].ea) s[i].ea(state);
    }
    for (let i = 0; i < s.length; i++) {
        if (s[i].aquatic) s[i].aquatic(state);
    }
    for (let i = 0; i < s.length; i++) {
        if (s[i].boost) s[i].boost(state);
    }

    if (state.offense == 0 && !has(state, s_beehive)) state.offense = Math.max(0, 8 - state.defense) / (15 + 30 * state.presence);
    let res = (state.offense * 2.5) + state.defense * 1;
    let excessHP = Math.abs(state.defense - state.offense);
    if (state.offense > state.defense) excessHP *= 2.5;
    excessHP += Math.abs(state.defense - state.offense);
    excessHP *= 8;
    excessHP /= 10 + 2 * res;
    excessHP += 0.8;
    if (excessHP > 0) {
    ...
    if (state.offense > 5) res += Math.min((state.offense - 5) ** 3 / 12, 12);
    res += state.additive;
    res += state.defense * (state.presence);
    state.res0 = res;
    if (res < 3) res += (3 - res) / 2;
    state.res = res;

    for (let i = 0; i < s.length; i++) {
        if (s[i].ext) s[i].ext(state);
    }
    state.res += state.additiveExt;
    state.res *= state.multiplicative;
    return state.res;
}
...
const costToPower = [
    [3, 4.5, 6, 7.5, 9, 11, 13, 15, 17, 19, 21.5, 24, 26.5],
    [3, 6, 9.5, 14.5, 25],
...
monoWeights[s_burrow.id] = 3;
monoWeights[s_tutor.id] = 4;
monoWeights[s_quills.id] = 2;
monoWeights[s_death_touch.id] = 3;
monoWeights[s_free_sac.id] = 3;
monoWeights[s_stinky.id] = 2;
monoWeights[s_sniper.id] = 3;
monoWeights[s_aquatic.id] = 1.5;
monoWeights[s_bifurcated.id] = 3;
monoWeights[s_rabbit.id] = 0.5;
monoWeights[s_brittle.id] = 0;
monoWeights[s_skele_spawner.id] = 0;

const pairWeights = { ...monoWeights };
pairWeights[s_brittle.id] = 20;

const poolsPerCard = {};

for (let i = 0; i < eligibleCards.length; i++) {
    const card = eligibleCards[i];
    let monoSigils = [], pairSigils = [];
    let monoProbs = [], pairProbs = [];
    let monoTotal = 0, pairTotal = 0;

    const basePower = costToPower[elToPosition(card.element)][card.cost];
    const minThresh = (basePower) * 1.24;
    const maxThresh = (basePower) * 1.36;

    function basicChecks(s) {
        return !(card.attack == 0 && sigilEstimates[s.id].uselessAt0) && !(card.attack == 1 && s == s_free_sac) && card.sigils.indexOf(s) == -1
    }

    for (let j = 0; j < eligibleSigils.length; j++) {
        const s = eligibleSigils[j];
        const t = [s, ...card.sigils];
        if (basicChecks(s) && checkAllowed(t)) {
            const _eval = correctPE(powerEstimate(card.attack, card.health, t), card.element, card.cost, t);
            if (_eval >= minThresh && _eval <= maxThresh) {
                monoSigils.push([s]);
                monoProbs.push(monoWeights[s.id]);
                monoTotal += monoWeights[s.id];
            }
            for (let k = 0; k < j; k++) {
                const s2 = eligibleSigils[k];
                const t2 = [s, s2, ...card.sigils];
                if (basicChecks(s2) && checkAllowed(t2)) {
                    const _eval2 = correctPE(powerEstimate(card.attack, card.health, t2), card.element, card.cost, t2);
                    if (_eval2 >= minThresh && _eval2 <= maxThresh) {
                        pairSigils.push([s, s2]);
                        pairProbs.push(pairWeights[s.id] * pairWeights[s2.id]);
                        pairTotal += pairWeights[s.id] * pairWeights[s2.id];
                    }
                }
            }
        }
    ...
    if (monoTotal != 0 && pairTotal != 0) {
        for (let i = 0; i < monoProbs.length; i++) {
            monoProbs[i] *= pairTotal / monoTotal;
        }
        monoTotal = pairTotal;
    }

    poolsPerCard[card.id] = {
        sigils: monoSigils.concat(pairSigils),
        weights: monoProbs.concat(pairProbs),
        total: monoTotal + pairTotal
    };
}
...
function buffedCards() {
    ...
    for (let i = 0; i < picks.length; i++) {
        const pool = poolsPerCard[picks[i].id];
        const ind = pickRandom(pool.weights, pool.total, 1);

        const modded = new ModdedCard(picks[i]);
        modded.extraSigs = pool.sigils[ind];
        results.push(modded);
    }
...
let est;
function updateOPAI() {
    est = powerEstimate(bcAtk, bcHP, sigilObjs);
}
