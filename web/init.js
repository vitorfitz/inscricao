...
class Sigil {
    init(coords = null, name = null, desc = null, initData = function (card, sigil) { }, hidden = false) {
        ...
        this.onPreAttack = []; // (me,attacker,attacked,memory)
        this.onPostAttack = []; // (me,attacker,attacked,memory)
...
async function replace(me, biggerMe) {
    ...
    anim.enqueue('removeCard', { card: me });
...
class SFrozen extends Sigil {
    init(futureMe, name = null, desc = null) {
        ...
        this.onCardDied.push(new Listener(listen_me, async function (me) {
            ...
            //     updateBlockActions();
            // }.bind(this),650)
            (GameCard.fromCard(this.futureMe, true)).place(me.pos, me.side, false, 500);
        }.bind(this)))
    }
}
