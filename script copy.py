...
image = cv2.cvtColor(cv2.imread('test.png'), cv2.COLOR_BGR2GRAY)
thresh=35
...
while y_old<image.shape[0]-0.5:
    ...
    while x_old<image.shape[1]-0.5:
        newImg[-1].append(image[round(y_old)][round(x_old)])
        x_old+=avg
    y_old+=avg

def create_image_from_array(array):
    img = Image.new('RGBA', (len(array[0]), len(array)))
    pixels = img.load()
